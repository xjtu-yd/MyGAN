...
def generator(image, image_dim, label, reuse_variable = None):
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variable) as scope:
        ...
        w1 = tf.get_variable(name="g_w1", shape=[image_dim, 140*140], dtype=tf.float32,
                             initializer=tf.truncated_normal_initializer(stddev=0.02))
        b1 = tf.get_variable("g_b1", [140*140], tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.02))
        g1 = tf.matmul(tf.cast(image, tf.float32), w1) + b1
        g1 = tf.reshape(g1, [-1, 140, 140, 1]) # [image_dim, 8,8,128]
        g1 = tf.nn.relu(g1)
        #print(g1.shape)

        #print(label.shape)

        label = tf.cast(tf.reshape(label, shape=[-1, 1, 10, 4]), tf.float32)  # 转换数据类型
        label = tf.tile(label, [1, 140, 14, 1])  # 进行张量扩张，对c的第二维和第三维分别重复x_init.shape[1], x_init.shape[2]次,第一维和最后一维保持不变。
       # print(label.shape)
       ...
       # print(g1.shape)
        ...
        w2 = tf.get_variable("g_w2", [3, 3, 5,64], tf.float32,
                             initializer=tf.truncated_normal_initializer(stddev=0.02))
        b2 = tf.get_variable("g_b2", [64], tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.02))

        g2 = tf.nn.conv2d(g1, w2, strides=[1, 20, 20, 1], padding="SAME")  # [image_dim, 112, 112, 128]
        g2 = g2 + b2
        #print(g2.shape)
        ...
        w3 = tf.get_variable("g_w3", [3, 3, 32, 64], tf.float32,
                               initializer=tf.truncated_normal_initializer(stddev=0.02))
       ...
       # print(g3.shape)
        ...
        w4 = tf.get_variable("g_w4", [3, 3, 4, 32], tf.float32,
                             initializer=tf.truncated_normal_initializer(stddev=0.02))
...
def discriminator(input_image, label, reuse = None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
        ...
        #print(shp)
        label = tf.cast(tf.reshape(label, [-1, 1, 10, 4]), tf.float32)
        #label = tf.cast(np.reshape(label, shape=tf.stack([batch_size, 1, 10, 4])), tf.float32)
        label = tf.tile(label, [1, 490, 1, 1])
...
both_label = tf.placeholder(tf.float32, [None, 10, 4], name = "both_label")
...
for g in range(start_epoch, 300000):
    ...
    if g % 1000 == 0 :
        ...
        real_image_batchy = real_image_batchy.reshape([-1, 10, 4])
        #print(real_image_batchy.shape)
