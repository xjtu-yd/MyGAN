...
def generator(image, label1, label2, is_training=True, reuse=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse) as scope:
        ...
        # [bz, 28, 28, 1]
        out = tf.nn.sigmoid(deconv2d(output_shape, net, 2, 4, 4, 2, 2, padding='SAME', name='g_dc4'))
        return out
...
def discriminator(input_image, label1, label2, is_training=True, reuse=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
        ...
        x = conv_cond_concat(input_image, y1)  # [bz, 28, 28, 12]
        x = conv_cond_concat(x, y2)  # [bz, 28, 28, 22]
        # [bz, 14, 14, 64]
        net = lrelu(conv2d(22, x, 64, 4, 4, 2, 2, padding="SAME", name='d_conv1'), name='d_l1')
        # [bz, 7, 7, 128]
...
d_placeholder = tf.placeholder(tf.float32, [None, 28, 28, 2], name="d_placeholder")
...
G = generator(g_placeholder, both_label1, both_label2, is_training=True)
D_Gr, D_conv_real, D_cls_real = discriminator(d_placeholder, both_label1, both_label2, is_training=True)
D_Gf, D_conv_fake, D_cls_fake = discriminator(G, both_label1, both_label2, is_training=True, reuse=True)
...
images_for_tensorboard = generator(g_placeholder, both_label1, both_label2, is_training=False)
...
for g in range(start_epoch, 300000):
    ...
    _, dLoss, __, gLoss = sess.run([d_trainner, D_loss, g_trainner, G_loss],
                                   feed_dict={g_placeholder: g_noise, d_placeholder: real_image_batch, both_label1: label1, both_label2: label2})

    if g % 1000 == 0:
        print(str(g) + " dLoss:" + str(dLoss) + "gLoss:" + str(gLoss))
        # print(real_image_batchy.shape)
        summary = sess.run(merged,
                           {g_placeholder: g_noise, d_placeholder: real_image_batch, both_label1: label1, both_label2: label2})
        writer.add_summary(summary, g)

        # print(real_image_batchy.shape)
        label = label1[0].reshape([-1, 10])
        g_noise2 = np.random.normal(0, 1, size=[1, g_dim])
        generated_images = generator(g_noise2, label, label, is_training=False, reuse=True)
        # print(real_image_batchy.shape)

        image = sess.run(generated_images, feed_dict={g_placeholder: g_noise2, both_label1: label, both_label2: label})
        ...
        im = image
        result = discriminator(d_placeholder, both_label1, both_label2, is_training=False, reuse=True)
        # print(real_image_batchy.shape)
        estimate = sess.run(result, {d_placeholder: im, both_label1: label, both_label2: label})
