...
def generator(image, label1, label2,label3,label4, is_training=True, reuse=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse) as scope:
        # batch_size = image.get_shape().as_list()[0]
        batch_size = tf.shape(image)[0]

        image = tf.concat([image, label1], 1)  # [bz,zdim+10]
        image = tf.concat([image, label2], 1)
        image = tf.concat([image, label3], 1)
        image = tf.concat([image, label4], 1)
        ...
        output_shape = tf.stack([batch_size, 14, 14, 4])
        # [bz, 28, 28, 1]
        out = tf.nn.sigmoid(deconv2d(output_shape, net, 4, 4, 4, 2, 2, padding='SAME', name='g_dc4'))
        return out
...
def discriminator(input_image, label1, label2, label3, label4,is_training=True, reuse=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
        # batch_size = input_image.get_shape().as_list()[0]
        batch_size = tf.shape(input_image)[0]
        ydim = tf.shape(label1)[-1]
        y1 = tf.reshape(label1, [batch_size, 1, 1, ydim])
        y2 = tf.reshape(label2, [batch_size, 1, 1, ydim])
        y3 = tf.reshape(label3, [batch_size, 1, 1, ydim])
        y4 = tf.reshape(label4, [batch_size, 1, 1, ydim])
        x = conv_cond_concat(input_image, y1)  # [bz, 28, 28, 14]
        x = conv_cond_concat(x, y2)  # [bz, 28, 28, 24]
        x = conv_cond_concat(x, y3)
        x = conv_cond_concat(x, y4)
        # [bz, 14, 14, 64]
        net = lrelu(conv2d(44, x, 64, 4, 4, 2, 2, padding="SAME", name='d_conv1'), name='d_l1')
        # [bz, 7, 7, 128]
...
both_label1 = tf.placeholder(tf.float32, [None, 10], name="both_label1")
both_label2 = tf.placeholder(tf.float32, [None, 10], name="both_label2")
both_label3 = tf.placeholder(tf.float32, [None, 10], name="both_label3")
both_label4 = tf.placeholder(tf.float32, [None, 10], name="both_label4")

G = generator(g_placeholder, both_label1, both_label2, both_label3, both_label4, is_training=True)
D_Gr, D_conv_real, D_cls_real = discriminator(d_placeholder, both_label1, both_label2, both_label3, both_label4, is_training=True)
D_Gf, D_conv_fake, D_cls_fake = discriminator(G, both_label1, both_label2,  both_label3, both_label4, is_training=True, reuse=True)
...
images_for_tensorboard = generator(g_placeholder, both_label1, both_label2, both_label3, both_label4, is_training=False)
images_for_tensorboard = tf.reshape(images_for_tensorboard, [-1, 28, 28, 1])
#print(images_for_tensorboard.shape)
#images_for_tensorboard = tf.transpose(images_for_tensorboard, (0, 3, 2, 1))
#images_for_tensorboard = images_for_tensorboard[:][0][:][:]
...
for g in range(start_epoch, 300000):
    ...
    real_image_batchx1, real_image_batchy1 = mnist.train.next_batch(batch_size)
    real_image_batchx1 = real_image_batchx1.reshape([-1, 28, 28, 1])
    label1 = real_image_batchy1.reshape([-1, 10])

    real_image_batchx2, real_image_batchy2 = mnist.train.next_batch(batch_size)
    real_image_batchx2 = real_image_batchx2.reshape([-1, 28, 28, 1])
    label2 = real_image_batchy2.reshape([-1, 10])

    real_image_batchx3, real_image_batchy3 = mnist.train.next_batch(batch_size)
    real_image_batchx3 = real_image_batchx3.reshape([-1, 28, 28, 1])
    label3 = real_image_batchy3.reshape([-1, 10])

    real_image_batchx4, real_image_batchy4 = mnist.train.next_batch(batch_size)
    real_image_batchx4 = real_image_batchx4.reshape([-1, 28, 28, 1])
    label4 = real_image_batchy4.reshape([-1, 10])

    real_image_batchx1 = np.transpose(real_image_batchx1, (0, 3, 1, 2))
    real_image_batchx2 = np.transpose(real_image_batchx2, (0, 3, 1, 2))
    real_image_batchx3 = np.transpose(real_image_batchx3, (0, 3, 1, 2))
    real_image_batchx4 = np.transpose(real_image_batchx4, (0, 3, 1, 2))
    real_image_batch = np.concatenate((real_image_batchx1, real_image_batchx2), axis=1)
    real_image_batch3 = np.concatenate((real_image_batchx3, real_image_batchx4), axis=1)
    ...
    _, dLoss, __, gLoss = sess.run([d_trainner, D_loss, g_trainner, G_loss],
                                   feed_dict={g_placeholder: g_noise, d_placeholder: real_image_batch, both_label1: label1, both_label2: label2, both_label3: label3, both_label4: label4})

    if g % 1000 == 0:
        print(str(g) + " dLoss:" + str(dLoss) + "gLoss:" + str(gLoss))
        # print(real_image_batchy.shape)
        summary = sess.run(merged,
                           {g_placeholder: g_noise, d_placeholder: real_image_batch, both_label1: label1, both_label2: label2, both_label3: label3, both_label4: label4})
        writer.add_summary(summary, g)

        # print(real_image_batchy.shape)
        label_1 = label1[0].reshape([-1, 10])
        label_2 = label2[0].reshape([-1, 10])
        label_3 = label3[0].reshape([-1, 10])
        label_4 = label4[0].reshape([-1, 10])
        g_noise2 = np.random.normal(0, 1, size=[1, g_dim])
        generated_images = generator(g_noise2, both_label1, both_label2, both_label3, both_label4, is_training=False, reuse=True)
        # print(real_image_batchy.shape)

        image = sess.run(generated_images, feed_dict={g_placeholder: g_noise2, both_label1: label_1, both_label2: label_1, both_label3: label_3, both_label4: label_4})
        image = np.transpose(image, (0, 3, 1, 2))
        for i in range(4):
            # tmp = np.expand_dims(image[0][i],axis=-1)
            # print(tmp.shape)
            plt.imshow(image[0][i].reshape([28, 28]), cmap='Greys')
            plt.savefig("./samples/filename" + str(g) + "_" + str(i) + ".png")
        # image_add = 0.25 * image[0][0][:][:] + 0.25 * image[0][1][:][:] + 0.25 * image[0][2][:][:] + 0.25 * image[0][3][:][:]
        image_add = 0.5 * image[0][0][:][:] + 0.5 * image[0][1][:][:]
        ...
        im = image
        result = discriminator(d_placeholder, both_label1, both_label2, both_label3, both_label4, is_training=False, reuse=True)
        # print(real_image_batchy.shape)
        estimate = sess.run(result, {d_placeholder: im, both_label1: label_1, both_label2: label_1, both_label3: label_3, both_label4: label_4})
