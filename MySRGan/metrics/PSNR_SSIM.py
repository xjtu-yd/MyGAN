...
def calculate_single(img_gt, img_gen):
...
def calculate_single_train(img_gt, img_gen):
    PSNR = calculate_psnr_train(img_gt, img_gen)
    SSIM = calculate_ssim_train(img_gt, img_gen)
    print('PSNR: {:.6f} dB, \tSSIM: {:.6f}'.format(PSNR, SSIM))
    return PSNR, SSIM


def calculate_dir(folder_GT, folder_Gen):
    crop_border = 4
    ...
    for i, img_path in enumerate(img_list):
        ...
        # im_Gen = cv2.imread(os.path.join(folder_Gen, base_name + '_rlt' + suffix + '.png')) / 255.
        im_GT = load_image(img_path)
        im_Gen = load_image(os.path.join(folder_Gen, base_name + '_rlt' + suffix + '.png'))
...
def calculate_psnr_train(target, ref):
